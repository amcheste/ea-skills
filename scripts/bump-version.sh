#!/usr/bin/env bash
# Bump semver in .claude-plugin/plugin.json, update CHANGELOG.md, commit, and tag.
#
# Usage:
...
usage() {
  ...
  echo "  Examples:" >&2
  echo "    $0 patch                  # 1.1.0 → 1.1.1" >&2
  echo "    $0 minor                  # 1.1.0 → 1.2.0" >&2
  echo "    $0 major                  # 1.1.0 → 2.0.0" >&2
...
# ── Increment mode ────────────────────────────────────────────────────────────
  ...
  if [[ ! "$BASE" =~ ^([0-9]+)\.([0-9]+)\.([0-9]+)$ ]]; then
    echo "ERROR: plugin.json contains invalid semver: '$CURRENT'" >&2
    exit 1
  fi
...
# Update version in plugin.json (preserves all other fields and formatting)
python3 - <<PYEOF
import json
...
# Update CHANGELOG: replace '## [Unreleased]' with the new release entry,
...
# Stage and commit
git -C "$REPO_ROOT" add "$PLUGIN_FILE" "$CHANGELOG_FILE"
